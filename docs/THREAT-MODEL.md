...
- The orchestrator rejects no-op work, unexpected files, excessive file counts, symlink output, Git metadata mutation, writes outside the isolated clone, runtime failures, timeouts, validation inventory drift, and failed required policies.
...
- The fixed Reviewer rejects base mismatch, unsupported or invalid candidates, unexpected scope, excessive files, incomplete Builder evidence, missing or duplicate validation evidence, and any failed required build or test.
